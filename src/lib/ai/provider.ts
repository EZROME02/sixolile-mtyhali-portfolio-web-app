...
export async function runAiProvider(request: AiRequest, env: AiProviderEnv, fetchImpl: typeof fetch = fetch): Promise<AiResult> {
  ...
  const response = await fetchImpl(`${baseUrl.replace(/\/$/, "")}/chat/completions`, {
    ...
    body: JSON.stringify({
      model,
      messages: [
        { role: "system", content: "You are EZROME AI. Distinguish generated analysis from verified facts. Do not invent sources or private information." },
  ...
  const payload = (await response.json()) as ProviderPayload & { choices?: Array<{ message?: { content?: unknown } }> };
