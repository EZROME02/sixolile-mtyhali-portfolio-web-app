      ...
      return {
        ...
        async run() {
          if (query.startsWith("INSERT INTO users")) {
            rows.set(`user:${values[1]}`, { id: values[0], email: values[1], password_hash: values[2] });
          }
          if (query.startsWith("INSERT INTO sessions")) {
            rows.set(`session:${values[0]}`, { id: values[0], user_id: values[1], expires_at: values[2] });
          }
          if (query.startsWith("INSERT INTO workspaces")) {
            rows.set(`workspace:${values[0]}`, {
              id: values[0], owner_id: values[1], name: values[2], data: values[3], updated_at: values[4],
            });
          }
          return {};
        },
...
describe("durable repositories", () => {
  test("creates users, sessions and owner-scoped workspaces", async () => {
    ...
    await repos.workspaces.upsert("workspace-1", "user-1", "My Workspace", "{}", new Date().toISOString());
